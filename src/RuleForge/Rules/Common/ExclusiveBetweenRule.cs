...
{
    public class ExclusiveBetweenRule<T> : IRule<T> where T : IComparable<T>
    {
        private readonly T _from;
        private readonly T _to;
        public string ErrorMessage { get; }

        public ExclusiveBetweenRule(T from, T to, string? errorMessage = null)
        {
            _from = from;
            _to = to;
            ErrorMessage = errorMessage ?? $"Value must be between {from} and {to} (exclusive)";
        }
